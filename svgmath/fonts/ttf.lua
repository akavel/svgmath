...
TTFFormatError'OpenType/CFF fonts are unsupported'  else
TTFFormatError'Not a TrueType file'  end
...
TTFFormatError'Required table '+tableTag+' missing in TrueType file'    end
...
TTFFormatError'Magic number in \'head\' table does not match the spec'  end
  skip(ff, 2)
  ...
  if pylua.op_is(subtableOffset, nil) then
    ...
    if pylua.op_is(subtableOffset, nil) then
TTFFormatErrorpylua.mod('Cannot use font \'%s\': no known subtable in \'cmap\' table', self.fullname)    elseif self.log then
...
TTFFormatErrorpylua.mod('Unsupported format in \'cmap\' table: %d', tableFormat)  end
...
TTFFormatErrorpylua.mod('Invalid indexToLocFormat value (%d) in \'head\' table', str(self.indexToLocFormat))  end
...
print('Usage: TTF.py <path to TTF file>')  end
end
