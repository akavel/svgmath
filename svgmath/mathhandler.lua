...
sax.ContentHandler-- SAX ContentHandler for converting MathML formulae to SVG.
...
endElementNS = function(self, elementName, qName)
  ...
  namespace, localname = elementName
  if namespace and namespace~=MathNS then
sax.SAXParseException('SAX parser error: namespace on opening and closing elements don\'t match', nil, self.locator)  end
  if pylua.op_is(self.currentNode, nil) then
sax.SAXParseException('SAX parser error: unmatched closing tag', nil, self.locator)  end
